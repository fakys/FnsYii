<div class="favorite-page">
    ...
    <?php foreach ($products as $value):?>
        <div class="product-favorite-container  product-favorite-<?=$value->id?>">
            <a href="<?=\yii\helpers\Url::to(['product/show', 'id'=>$value->id])?>" class="product-favorite-image">
                <img src="<?=Yii::getAlias('@web')."/{$value->main_photo}"?>">
            </a>
            <div class="product-favorite-content">
                <a href="<?=\yii\helpers\Url::to(['product/show', 'id'=>$value->id])?>" class="link-product"><?= strlen($value->title)>=70? substr($value->title, 0 ,70).'...':$value->title?></a>
                <a href="<?=\yii\helpers\Url::to(['product/show', 'id'=>$value->id])?>" class="link-product-active"><?= strlen($value->description)>=200? substr($value->description, 0 ,200).'...':$value->description?></a>
            </div>
            <div class="d-flex flex-column">
                <a href="<?=\yii\helpers\Url::to(['product/show', 'id'=>$value->id])?>" class="price-favorite">
                    <?=$value->price?> ₽
                </a>
